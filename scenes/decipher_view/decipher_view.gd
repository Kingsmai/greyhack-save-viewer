# ------------------------------------------------
# Class Name: DecipherView
# Class Description:
# ------------------------------------------------
class_name DecipherView extends MarginContainer
...
func _ready() -> void:
	reload_button.pressed.connect(func(): load_decipher_view())
	decrypt_button.pressed.connect(_on_decrypt_button_pressed)

func load_decipher_view():
	PasswordsRepository.reload_password_dictionary()
	saved_password = PasswordsRepository.get_password_dictionary()
...
func _on_decrypt_button_pressed() -> void:
	...
	for encrypted_text in encrypted_texts:
		...
		if base64password in saved_password:
			plain_password = saved_password[base64password]
		else:
